...
# -------------------------
def load_data(client_file="client3.csv", test_split_ratio=0.5):
    try:
...
def test_model(model, testloader, device="cpu"):
        model.to(device)
        model.eval()
        criterion = torch.nn.BCEWithLogitsLoss(reduction="sum")  # sum to accumulate then divide
        total_loss = 0.0
        total_samples = 0
...
class FlowerADMMClient(NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # lambda may be provided as a JSON string in config; y is provided via parameters and already loaded.
...
# -------------------------
def client_fn(cid: str, csv_path="client3.csv"):
    return FlowerADMMClient(client_csv_path=csv_path).to_client()

if __name__ == "__main__":
    # Example CLI usage for a single client process:
    # python admm_flower_client.py client3.csv
    import sys
    from flwr.client import start_client

    client_csv = "client3.csv" if len(sys.argv) < 2 else sys.argv[1]

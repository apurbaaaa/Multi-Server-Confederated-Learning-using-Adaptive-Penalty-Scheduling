...
# -------------------------
def load_data(client_file="client6.csv", test_split_ratio=0.5):
    try:
...
# -------------------------
def client_fn(cid: str, csv_path="client6.csv"):
    return FlowerADMMClient(client_csv_path=csv_path).to_client()

if __name__ == "__main__":
    # Example CLI usage for a single client process:
    # python admm_flower_client.py client6.csv
    import sys
    from flwr.client import start_client

    client_csv = "client6.csv" if len(sys.argv) < 2 else sys.argv[1]
